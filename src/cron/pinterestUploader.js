...
const uploadHistory = [];
const homeUrl = 'https://www.pinterest.at/perplexedart/';
let lastUrl = '';
let randomProfileUrls = [];

exports.follow = async (page,followProbability) => {
  if (!(await goToUrl(page,homeUrl))) {
    console.log('pinterestFollow: Failed loading home page.');
    return false;
  }

  if (!(await unfollow(page))) {
    console.log('pinterestFollow: Failed unfollow.');
    return false;
  }

  if (!(await fillRandomProfileUrls(page))) {
    console.log('pinterestFollow: Failed fillRandomProfileUrls.');
    return false;
  }

  const profileDetails = await searchFittingProfileAndGetDetails(page);
  if (!profileDetails) {
    console.log('pinterestFollow: Failed searchFittingProfileAndGetDetails.');
    return false;
  }

  if (!(await followLikeComment(page,profileDetails,followProbability))) {
    console.log('pinterestFollow: Failed followLikeCommentPosts.');
    return false;
  }

  console.log('pinterestFollow: Successfully followed/liked/commented profile "' + profileDetails.url.split('.com/')[1] + '".');
  return true;
}
...
exports.uploadNext = async (page) => {
  const image = await imageModel.getNextToUpload('pinterest');
  console.log('AAAAA',image);
  if (!image) {
    console.log('pinterestUploader: No next image to upload.');
    return false;
  }
  console.log('pinterestUploader: Uploading image: ' + fct.limitString(image.promptText,20));

  const category = await categoryModel.get(image.categoryName);
  if (!category) {
    console.log('pinterestUploader: Failed getting category of image.');
    return false;
  }

  if (!(await goToUrl(page,homeUrl))) {
    console.log('pinterestUploader: Failed loading home page .');
    return false;
  }

  if (!(await fillCreateForm(page,category,image))){
    console.log('pinterestUploader: Failed fillform .');
    return false;
  }

  const success = await fct.waitForElementWithTagAndFieldValue(page,'span','innerHTML','Your post has been shared.',40);
  if (!success) {
    console.log('pinterestUploader: Failed creation .');
    return false;
  }

  await imageModel.set(image.categoryName, image.promptText, 'pinterestUploaded', true);

  if (!(await saveImageUrl(page,image))) {
    console.log('pinterestUploader: Failed saveImageUrl .');
    return false;
  }

  uploadHistory.push(image);
  console.log('pinterestUploader: Successfully uploaded image "' + fct.limitString(image.promptText, 30) + '"');
  return true;
}
